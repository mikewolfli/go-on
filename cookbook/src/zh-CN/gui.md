...
- **风险决策面板**：展示高风险 AI 回复的风险评估、缓解策略和审查要求（详见下方专题说明）
...
当后端治理层检测到潜在高风险内容时，GUI 会在 AI 响应中显示**风险决策面板**。该功能属于 Safeguard（安全审查）模式和多模型投票系统的一部分。

**触发机制：** 后端将用户消息与领域关键词（医疗、法律、金融、安全等）及决策相关关键词进行比对分析。当风险评分超过阈值时，后端激活一项或多项缓解策略：

- **多模型投票 (Multi-model vote)**：将请求同时发送给多个模型，比较输出结果
- **多智能体投票 (Multi-agent vote)**：将请求路由到不同 AI 智能体，汇总结果
- **升级处理 (Escalation)**：使用更强大（也更贵）的模型进行最终判断
- **需要审查 (Review required)**：标记响应以供人工审核

**GUI 显示内容：**
- 风险状态：**高风险** 或 **正常**
- 使用策略：如 "multi_model_vote"、"multi_agent_vote"、"escalation"
- 审查要求指示
- 具体风险原因（最多显示 4 条）

面板自适应主题：高风险话题显示暖橙色/红色背景，正常风险显示轻微绿色背景——两种模式均正确支持深色/浅色主题。

### 设置面板 (Settings)
- **Provider 管理**：动态环境变量注入（支持全部 37 家供应商），不再硬编码为 8 个
- **配置文件编辑器**：管理 `gui_config.json`，含 JSON 语法验证
- **主题选择**：6 种视觉主题（简约 / 国风 / 武侠 / 山水 / Hello Kitty / 为人民服务）
