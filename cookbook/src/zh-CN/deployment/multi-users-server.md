...
### 特性标志
多用户服务器模式是一个 **profile**（`multi-users-server`），已包含 PostgreSQL
后端；单独启用 `backend-postgres` 特性不会选择任何 profile，会触发编译期门禁失败。
具体 profile 组成见 `Cargo.toml`。
...
### 为企业部署构建
```bash
# 使用 multi-users-server profile 构建（包含 PostgreSQL + pgvector）
