...
- **Backend**: the executable owns config loading, provider selection, routing, setup, health checks, protocol negotiation, HTTP or stdio transport, and a 7-feature-gated sub-bus capability architecture with cognitive modules. All 154 JSON-RPC methods (the `ACP_METHODS` whitelist) return a unified `DispatchOutput` enum.
...
All 154 JSON-RPC methods (the `ACP_METHODS` whitelist) return `Result<DispatchOutput>`. The `dispatch_to_client` function converts each variant to the appropriate transport response:
