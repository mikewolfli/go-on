...
- Config templates live in `config/config.toml` (local), `config/config.simple-server.toml`, `config/config.multi-users-server.toml` (server profiles). There is no separate `config.toml.autopilot-adaptive` template — the adaptive config is generated by `src/core/setup/config_gen.rs` at onboarding/`--setup` time.
