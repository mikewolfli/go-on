...
- Config templates live in `config/config.toml` (local), `config/config.simple-server.toml`, `config/config.multi-users-server.toml` (server profiles), `config/config.low-memory.toml` (memory-constrained systems), and `config/zed-config.toml` (Zed ACP agent server). There is no separate `config.toml.autopilot-adaptive` template — the adaptive config is generated by `src/core/setup/config_gen.rs` at onboarding/`--setup` time.
- `AppConfig::validate()` is the source of truth for constraints.
- Adding new config fields requires:
  - struct update in `src/core/config/types.rs`
  - validation update in `src/core/config_validation.rs` / `AppConfig::validate()`
  - template update in the `config/config*.toml` files
