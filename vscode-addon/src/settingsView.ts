...
export class GoOnSettingsViewProvider implements vscode.WebviewViewProvider {
    ...
    private async _handleKeyringGet(name: string) {
        try {
            const value = await vscode.commands.executeCommand<string>('go-on.keyringGet', { name });
            this._postMessage({
                type: 'keyringResult',
    ...
    private async _handleKeyringList() {
        try {
            const output = await vscode.commands.executeCommand<string>('go-on.keyringList');
            this._postMessage({
                type: 'keyringResult',
    ...
    private async _handleApplyDefaultConfigTemplate(template: string) {
        try {
            const configPath = await vscode.commands.executeCommand<string>('go-on.applyDefaultConfigTemplate', { template });
            this._postMessage({
    ...
    private async _handleApplyRulesSettings(payload: {
        ...
        try {
            const rulesDir = await vscode.commands.executeCommand<string>('go-on.updateRules', payload);
            this._postMessage({
                type: 'settingsActionResult',
    ...
    private async _handleApplyWorkflowMapping(payload: {
        ...
        try {
            const configPath = await vscode.commands.executeCommand<string>('go-on.updateWorkflowMapping', payload);
            this._postMessage({
                type: 'settingsActionResult',
