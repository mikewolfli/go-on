...
export interface RuntimeBootstrapDeps {
    ensureBinary: (_workspaceRoot: string | undefined, _config: vscode.WorkspaceConfiguration, _context: vscode.ExtensionContext) => Promise<unknown>;
    isRunning: () => boolean;
...
export async function ensureGoOnStarted(isRunning: () => boolean, startCommandId: string): Promise<void> {
    if (isRunning()) {
        return;
    }
...
export function buildPlaceholderEnvValues(envNames: string[]): Record<string, string> {
