...
export class GoOnAdvancedEditProvider {
    ...
    private createExplainCodeAction(document: vscode.TextDocument, range: vscode.Range): vscode.CodeAction {
    ...
    private createRefactorCodeAction(document: vscode.TextDocument, range: vscode.Range): vscode.CodeAction {
    ...
    private createOptimizeCodeAction(document: vscode.TextDocument, range: vscode.Range): vscode.CodeAction {
    ...
    private async handleResultOutput(resultText: string, editor: vscode.TextEditor, selection: vscode.Range, language: string) {
        const config = vscode.workspace.getConfiguration('go-on');
        const showDiffByDefault = config.get<boolean>('advancedAI.showDiffByDefault', true);
    ...
    private buildPrompt(action: string, code: string, language: string, details?: string): string {
        const config = vscode.workspace.getConfiguration('go-on');
        const advancedContext = config.get<string>('advancedAI.context', 'You are a senior engineer assistant with expertise across languages, security, testing, and performance.');
    ...
    private buildRefactorPrompt(refactorType: string, code: string, language: string): string {
        const prompts: { [key: string]: string } = {
