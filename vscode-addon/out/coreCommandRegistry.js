...
function registerCoreCommands(deps) {
    const diagnoseCommand = vscode.commands.registerCommand("go-on.diagnose", async () => {
        ...
        try {
            const resolvedConfigPath = await deps.resolveConfigPath(workspaceFolder.uri.fsPath, configuredConfigPath, runtimeDir || workspaceFolder.uri.fsPath);
        ...
        try {
            ...
            if (missingEnvVars.length > 0) {
                try {
                    ...
                    const retryMissingEnvVars = deps.parseMissingEnvVariableNames(retryMessage);
                    if (retryMissingEnvVars.length > 0) {
                        vscode.window.showErrorMessage(`Failed to start Go-On: missing environment variables (${retryMissingEnvVars.join(", ")}). Configure provider keys in Settings.`);
                    }
                    else {
    ...
    const lockStatusCommand = vscode.commands.registerCommand("go-on.lockStatus", async () => {
        ...
        try {
            ...
            const topLabel = contentionTop.length > 0
                ...
                : "-";
            vscode.window.showInformationMessage(`lock.status: status=${String(locks.status ?? "unknown")}, tracked=${Number(locks.components_tracked ?? 0)}, poisoned=${Number(locks.poisoned_total ?? 0)}, recovered=${Number(locks.recovered_total ?? 0)}, slow_waits=${Number(locks.slow_wait_total ?? 0)}, max_wait_ms=${Number(locks.max_wait_ms ?? 0).toFixed(3)}, top=${topLabel}`);
        }
        catch (error) {
