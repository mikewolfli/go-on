...
function registerRpcCommands(deps) {
    ...
    const configBaselineRpcCommand = vscode.commands.registerCommand('go-on.configBaseline', async () => {
        ...
        try {
            ...
            vscode.window.showInformationMessage(`config.baseline: status=${status}, mode=${configuredMode}, capability=${capability}, dispatch=${dispatch}, transport=${transport}, strict=${strictEnabled ? 'on' : 'off'}, runtime_explicit=${explicitCount}, legacy_keys=${legacyCount}`);
        }
        catch (error) {
    ...
    const releaseReadinessRpcCommand = vscode.commands.registerCommand('go-on.releaseReadiness', async () => {
        ...
        try {
            ...
            const blockedGateNames = asArray(readiness.blocked_gate_names)
                ...
                .join('|');
            const readinessSchema = String(readiness.schema_version ?? artifactContract.schema_version ?? readiness.version ?? 'unknown');
            vscode.window.showInformationMessage(`release.readiness: status=${String(readiness.status ?? 'unknown')}, schema=${readinessSchema}, overall=${Boolean(readiness.overall_pass)}, blocked=${Number(readiness.blocked_gate_count ?? 0)}, blocked_names=${blockedGateNames || '-'}, open_breakers=${Number(summary.open_breakers ?? 0)}, degraded_services=${Number(summary.degraded_services ?? 0)}, multi_user_mode=${String(multiUser.mode ?? 'single_user')}, multi_user_ready=${Boolean(multiUser.release_gate_ready ?? false)}, multi_user_lifecycle_ready=${Boolean(lifecycle.ready ?? summary.multi_user_lifecycle_ready ?? false)}, dual_track_ready=${Boolean(dualTrack.ready ?? summary.dual_track_consistency_ready ?? false)}, multi_user_source=${String(inference.source ?? summary.multi_user_inference_source ?? 'default')}`);
        }
        catch (error) {
