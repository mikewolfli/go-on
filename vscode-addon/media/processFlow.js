(function() {
    ...
    function renderProcessFlow() {
        ...
        processCanvas.onmouseup = () => { draggingNode = null; };
        processCanvas.onmouseleave = () => { draggingNode = null; };
    ...
    createProcessBtn.addEventListener('click', async () => {
        const name = await vscode.window.showInputBox({ prompt: 'Enter process name' });
        if (!name) return;

        const stageCount = await vscode.window.showInputBox({ prompt: 'Stage count (default 3)' });
        const count = Number(stageCount || '3') || 3;
        ...
        vscode.postMessage({ type: 'createProcess', processData: { name, stages, status: 'created' } });
    });

    runProcessBtn.addEventListener('click', () => {
        if (!selectedProcessId) {
            vscode.window.showWarningMessage('Select a process first');
            return;
        }
        ...
        switch (message.type) {
            ...
            case 'stageStatusUpdate':
                if (processes[message.processId]) {
                    ...
                    process.stages[message.stageIndex] = process.stages[message.stageIndex] || {};
