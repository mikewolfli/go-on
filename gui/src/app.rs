    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if self.connection.backend_child.is_none() && !self.show_setup {
            if let Some(crash_time) = self.crash.backend_crash_time {
                let backoff_secs = self.crash.backoff_secs(); // 3, 6, 12, 24, 48, 96
                    ...
                    .show(ui, |ui| {
                        ...
                        match self.active_tab.as_str() {
                            ...
                            "skills" => {
                                ...
                                if self.ui_state.skills_show_create
                                    != self.views.skills_view.show_create
                                    || self.ui_state.skills_show_import
                                        != self.views.skills_view.show_import
                                {
                                    self.ui_state.skills_show_create =
                                        self.views.skills_view.show_create;
                                    self.ui_state.skills_show_import =
                                        self.views.skills_view.show_import;
                                    self.ui_state.save();
                            ...
                            "prompts" => {
                                self.views.prompts_view.show(ui, &self.i18n);
                            }
                            "risk_decision" => {
                                let draft = self.views.chat_view.risk_decision_draft();
                                self.views.risk_decision_view.apply_draft(&draft);
                                if let Some(block) =
                                    self.views.risk_decision_view.show(ui, &self.i18n)
                                {
                                    if self.views.chat_view.input.trim().is_empty() {
                            ...
                            "security" => self.views.security_view.show(
                                ui,
                                &self.i18n,
                                &self.connection.backend,
                                ctx,
                            ...
                            "about" => {
                                self.views.about_view.show(
                                    ui,
                                    &self.i18n,
                                    self.views.monitor_view.health.as_ref(),
                                    self.connection
                                        .backend_child
                                        .as_ref()
