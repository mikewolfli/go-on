...
/// Replace a single `key = "..."` line inside a TOML source string.
///
/// Keeps the original indentation and only touches the first line whose
/// trimmed form starts with `key`. Used when overlaying GUI-specific values
/// (e.g. the ACP bind address) onto the `config/config.toml` base.
fn replace_toml_line(source: &str, key: &str, new_line: &str) -> String {
...
impl GoOnApp {
    ...
    pub(crate) fn spawn_backend(
        ...
        match find_backend_binary() {
            Some(path) => {
                ...
                // and the backend resolves these via system keyring. This ensures secrets
    ...
    pub(crate) fn generate_backend_config(path: &std::path::Path, config: &AppConfig) {
        // == Validation helpers available below ==
...
"#
        ...
        let static_part = replace_toml_line(
            static_part,
            "acp_http_bind_addr",
            &format!("acp_http_bind_addr = \"{}\"", bind_addr),
        );

        // `[protocol].mode` is the canonical key (the backend syncs it onto
        // `runtime.protocol_mode` during config load).
        let toml = format!(
            "# Auto-generated by go-on-gui — do not edit manually.\n# Provider settings are managed from the GUI's Providers/Settings page.\n\n{}[protocol]\nmode = \"{}\"\n\n{}\n",
            static_part,
