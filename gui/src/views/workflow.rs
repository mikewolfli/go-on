...
thread_local! {
    static WORKFLOW_CACHE: RefCell<CachedView> = RefCell::new(CachedView::new());
}
                ...
                .check_or_render(ui, "workflow_view", hash, |ui| {
                    egui::ScrollArea::vertical()
                        ...
                        .show(ui, |ui| {
                            ...
                            if self.security_last_load.elapsed()
                                >= std::time::Duration::from_secs(10)
                            {
                            ...
                            for (idx, step) in self.state.steps.iter_mut().enumerate() {
                                egui::Frame::group(ui.style()).show(ui, |ui| {
                                    ui.horizontal(|ui| {
                                        ...
                                        let delete_label =
                                            if self.pending_confirm_delete == Some(idx) {
                                        ...
                                        ui.horizontal(|ui| {
                                            for (label, action) in available_actions {
                                                if ui.button(label).clicked() {
                                                    ...
                                                    let detail_request_id =
                                                        self.run_detail_request_seq;
                                                    self.run_detail_in_flight = true;
                                                    self.last_requested_run_detail_id =
                                                        run_id.clone();
                                                    let backend_clone = backend.clone();
                                                    let tx = self.pending_tx.clone();
                                                    let ctx_clone = ctx.clone();
                                                    ...
                                                    "Warning: transition_workflow_run timed out"
                                                        ...
                                                        if let Err(e) = tx
                                                            .try_send(WorkflowEvent::UiMessage(msg))
                                                        {
                                                            eprintln!(
