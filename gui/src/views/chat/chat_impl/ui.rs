    ...
    fn show_safe_chat_layout(
        ...
        ui.horizontal(|ui| {
            ...
            ui.allocate_ui(egui::vec2(content_w, total_h), |ui| {
                ...
                if CHAT_STAGE6_ENABLE_EXTRA_BUTTONS && self.show_prompts {
                    ui.group(|ui| {
                        ui.label(i18n.t("chat.promptTemplates"));
                        let mut insert_content: Option<String> = None;
                        for template in &self.prompt_templates {
                            if ui
                                .button(format!("{}  {}", template.command, template.name))
                                .clicked()
                            {
                                insert_content = Some(template.content.clone());
                            }
                        }
                        if let Some(content) = insert_content {
                            self.input = content;
                            self.show_prompts = false;
                        }
                    });
