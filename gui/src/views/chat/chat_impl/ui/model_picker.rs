...
pub fn render_model_picker(chat: &mut ChatView, ui: &mut egui::Ui, _i18n: &I18n) -> bool {
    let prev_model = chat.model_state.selected_model.clone();

    let agent_keys: Vec<String> = chat
        .model_state
    ...
    let agent_text = if chat.model_state.selected_agent.is_empty() {
        "All Agents".to_string()
    } else {
        chat.model_state.selected_agent.clone()
    };
    egui::ComboBox::from_id_salt("agent_sel")
        ...
        .show_ui(ui, |ui| {
            if ui
                .selectable_value(
                    &mut chat.model_state.selected_agent,
                    String::new(),
                    "All Agents",
                )
                .clicked()
            {
                ...
                if chat.model_state.selected_model.is_empty()
                    || chat.model_state.selected_model == "auto"
                {
                    chat.model_state.selected_model = "auto".to_string();
                }
            ...
            for agent in &agent_keys {
                    ...
                    .clicked()
                {
                    ...
                    let current_model = chat.model_state.selected_model.clone();
                    let agent_models = chat.model_state.available_agent_models.get(agent);
                    ...
                    if model_ok {
                        ...
                        if let Some(first) = models.first() {
                            chat.model_state.selected_model = first.clone();
                        } else {
                            chat.model_state.selected_model = "auto".to_string();
                        }
                    } else {
    ...
    let model_options: Vec<String> = if chat.model_state.selected_agent.is_empty() {
        let mut all: Vec<String> = chat.model_state.available_models.clone();
        ...
        let mut agent_models = chat
            .model_state
            .available_agent_models
            .get(&chat.model_state.selected_agent)
            .cloned()
            .unwrap_or_default();
    ...
    egui::ComboBox::from_id_salt("model_sel")
        .selected_text(if chat.model_state.selected_model == "auto" {
            "AUTO".to_string()
        } else if chat.model_state.selected_model == ChatView::COPILOT_AUTO_MODEL {
            "copilot/auto".to_string()
        } else {
            chat.model_state.selected_model.clone()
        })
        .show_ui(ui, |ui| {
            for m in &model_options {
                ...
                ui.selectable_value(&mut chat.model_state.selected_model, m.to_string(), label);
            }
        });

    chat.model_state.selected_model != prev_model
}
