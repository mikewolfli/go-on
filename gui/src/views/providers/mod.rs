...
/// GUI-side model suggestions per provider.
///
/// Delegates to the generated catalog, which mirrors the backend's
/// `ProviderSpec::model_suggestions` (single source of truth — see
/// `scripts/gen-provider-catalog.py`). The backend agents' `available_models()`
/// fetched live remains the authoritative runtime source; this is the offline
/// fallback. The UI-only `"auto"` sentinel is not included — callers prepend
/// it themselves.
pub(crate) fn models_for_provider(provider: &str) -> &'static [&'static str] {
    catalog::default_models(provider)
}
