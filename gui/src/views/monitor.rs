    ...
    pub fn show(
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                ...
                let mut hash =
                    hash_combine(hash_bool(self.backend_configured), hash_str(&self.error));
                ...
                if let Some(cached_size) = self.cached_view.check_size("monitor", hash) {
                    ...
                    let resp = egui::Frame::NONE.show(ui, |ui| {
                        ...
                        if monitor_history_alerts_enabled {
                            egui::Frame::group(ui.style()).show(ui, |ui| {
                                            ...
                                            let payload = match result {
                                                Ok((groups, failures)) => {
                                                    ...
                                                    (
                                                        format!("__metrics__:{metrics}"),
                                                        format!(
                                                            "__errors_summary__:{summary_json}"
                                                        ),
                                                    )
                                                }
                                                Err(e) => (
                                ...
                                if !self.error_groups.is_empty() {
                                    ...
                                    ui.label(i18n.t("monitor.sampleFailures").replace(
                                        "{count}",
                        ...
                        if self.backend_configured
                            && self.health.as_ref().is_none_or(|h| !h.connected)
                        {
                            ui.colored_label(
                    ...
                    self.cached_view
                        .store_size("monitor", hash, resp.response.rect.size());
                }
            });
    }
}
