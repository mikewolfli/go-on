...
fn load_blue5_doc_lazy(config_path: Option<&PathBuf>) -> Blue5DocSnapshot {
    ...
    let supports_requirement_codiscussion = enabled
        && (content.contains("鐩爣 F")
            || content_lower.contains("requirement co-discussion")
            || content.contains("澶氳疆璁ㄨ"));
    let supports_consultation =
        enabled && (content_lower.contains("consultation") || content.contains("浼氳瘖"));
...
async fn run_consultation_workflow(
    ...
    let looks_uncertain = reviewer_lower.contains("鏃犳硶")
        || reviewer_lower.contains("涓嶇‘瀹?)
        ...
        if !bypass_cache && cache_enabled {
            ...
            if let Some(cache) = self.cache_handle() {
                ...
                debug!(
                    trace_id = %pipeline_trace.trace_id,
                    phase = %routing.phase.phase_name,
                    "sqlite cache miss 鈥?forwarding to live agent"
                );
            }
        }
        ...
        let mut errors: Vec<String> = Vec::new();
                ...
                .await
            {
                Ok(response_text) => {
                    ...
                    // Section 7: QA gate 鈥?only for FullAuto + high-complexity requests
                            ...
                            Ok(qa_report) => {
                                if !qa_report.ok {
                                    warn!(
                                        ...
                                        "qa gate: artifacts incomplete 鈥?checkpoint and retest before promotion"
                                    );
                                }
                                let _ = self
                                    .send_notification(
...
fn requirement_questions_from_missing(missing_fields: &[String]) -> Vec<String> {
    missing_fields
        ...
        .map(|field| match field.as_str() {
            "goal" => "杩欎釜浠诲姟鏈€缁堟兂杈炬垚鐨勪笟鍔＄洰鏍囨槸浠€涔堬紵".to_string(),
            "scope" => "鏈鏀瑰姩杈圭晫鏄粈涔堬紵鍝簺妯″潡蹇呴』鍖呭惈锛?.to_string(),
            "acceptance_criteria" => "楠屾敹鏍囧噯鏄粈涔堬紵濡備綍璇佹槑瀹屾垚锛?.to_string(),
            "constraints" => "鏈夊摢浜涚‖绾︽潫锛堟椂闂淬€佸吋瀹规€с€佹€ц兘銆佸畨鍏級锛?.to_string(),
            other => format!("璇疯ˉ鍏呭瓧娈? {}", other),
        })
        .collect::<Vec<_>>()
}
