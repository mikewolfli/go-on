...
impl Default for RuntimeConfig {
    fn default() -> Self {
        Self {
            ...
            // Deliberately None (matches the serde default): when otel_enabled
            // is true, init_otel_export falls back to OTEL_EXPORTER_OTLP_ENDPOINT
            // and otherwise refuses to export with a warning. A hard-coded
            // "http://localhost:4317" here only applies to unwrap_or_default()
            // paths (tests/CLI helpers) and would silently diverge from the
            // parsed-config path, which stays None.
            otel_endpoint: None,
...
pub fn default_non_ai_config_toml() -> String {
    // Cache/vector/runtime values are injected from the canonical default
    // functions above so this bootstrap template can never drift from the
    // serde defaults (previously re-typed as literals in three places).
    [
        ...
        format!("path = \"{}\"", default_cache_path()),
        format!("default_ttl_seconds = {}", default_cache_ttl_seconds()),
        format!("max_entries = {}", default_cache_max_entries()),
        String::new(),
        "[vector]".to_string(),
        "enabled = true".to_string(),
        format!("auto_mode = {}", default_vector_auto_mode()),
        format!("path = \"{}\"", default_vector_path()),
        format!("dimensions = {}", default_vector_dimensions()),
        format!("min_query_chars = {}", default_vector_min_query_chars()),
        format!("top_k = {}", default_vector_top_k()),
        format!("min_similarity = {}", default_vector_min_similarity()),
        format!("max_snippet_chars = {}", default_vector_max_snippet_chars()),
        format!("max_entries = {}", default_vector_max_entries()),
        format!("summary_enabled = {}", default_summary_enabled()),
        format!(
            "summary_trigger_messages = {}",
            default_summary_trigger_messages()
        ),
        format!("summary_max_chars = {}", default_summary_max_chars()),
        String::new(),
        "[runtime]".to_string(),
        format!(
        ...
        format!(
            "evaluate_interval = {}",
            crate::config::autotune::default_autotune_evaluate_interval()
        ),
        format!(
            "min_query_chars_step = {}",
            crate::config::autotune::default_autotune_min_query_chars_step()
        ),
        format!(
            "min_query_chars_min = {}",
            crate::config::autotune::default_autotune_min_query_chars_min()
        ),
        format!(
            "min_query_chars_max = {}",
            crate::config::autotune::default_autotune_min_query_chars_max()
        ),
        format!(
            "max_top_k = {}",
            crate::config::autotune::default_autotune_max_top_k()
        ),
        format!(
            "low_precision_threshold = {}",
            crate::config::autotune::default_autotune_low_precision()
        ),
        format!(
            "high_precision_threshold = {}",
            crate::config::autotune::default_autotune_high_precision()
        ),
        format!(
            "state_path = \"{}\"",
            crate::config::autotune::default_autotune_state_path()
        ),
        format!(
            "cooldown_windows = {}",
            crate::config::autotune::default_autotune_cooldown_windows()
        ),
        format!(
            "min_vector_searches = {}",
            crate::config::autotune::default_autotune_min_vector_searches()
        ),
        format!(
            "summary_trigger_min = {}",
            crate::config::autotune::default_autotune_summary_trigger_min()
        ),
        format!(
            "summary_trigger_max = {}",
            crate::config::autotune::default_autotune_summary_trigger_max()
        ),
        String::new(),
        "[agents]".to_string(),
        String::new(),
        "[flow]".to_string(),
        "name = \"Autopilot Adaptive\"".to_string(),
        "workflow_type = \"auto\"".to_string(),
        "phases = [\"planning\", \"coding\", \"review\", \"delivery\"]".to_string(),
        String::new(),
        "[phases.planning]".to_string(),
        "description = \"Planning phase\"".to_string(),
        "agents = []".to_string(),
        "fallback = true".to_string(),
        String::new(),
        "[phases.coding]".to_string(),
        "description = \"Coding phase\"".to_string(),
        "agents = []".to_string(),
        "fallback = true".to_string(),
        String::new(),
        "[phases.coding.options]".to_string(),
        "autopilot_complexity = \"auto\"".to_string(),
        "request_timeout_seconds = 150".to_string(),
        "review_timeout_seconds = 60".to_string(),
        "cache_enabled = true".to_string(),
        "vector_enabled = true".to_string(),
        "summary_enabled = true".to_string(),
        "full_auto_review_agents = []".to_string(),
        "phase_max_inflight = 24".to_string(),
        "global_max_inflight = 128".to_string(),
        String::new(),
        "[phases.review]".to_string(),
        "description = \"Review phase\"".to_string(),
        "agents = []".to_string(),
        "fallback = true".to_string(),
        String::new(),
        "[phases.review.options]".to_string(),
        "request_timeout_seconds = 60".to_string(),
        "review_timeout_policy = \"reject\"".to_string(),
        "review_min_response_chars = 12".to_string(),
        "phase_max_inflight = 16".to_string(),
        "global_max_inflight = 128".to_string(),
        String::new(),
        "[phases.delivery]".to_string(),
        "description = \"Delivery phase\"".to_string(),
        "agents = []".to_string(),
        "fallback = false".to_string(),
        String::new(),
        "[phases.delivery.options]".to_string(),
        "request_timeout_seconds = 90".to_string(),
    ]
    .join("\n")
}
