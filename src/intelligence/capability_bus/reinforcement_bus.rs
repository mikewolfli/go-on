...
mod tests {
    ...
    fn test_select_action() {
        ...
        bus.set_exploration_rate(0.0); // deterministic selection
        let actions = vec![
            "code".to_string(),
            "research".to_string(),
            "review".to_string(),
    ...
    fn test_record_reward_updates_q_table() {
        let mut bus = ReinforcementBus::new();
        bus.set_exploration_rate(0.0);
        assert_eq!(bus.table_size(), 0);

        bus.record_reward("s1", "a1", 1.0, "s2");
        // Q(s1, a1) should now be present in the table
        assert_eq!(bus.table_size(), 1);
        assert!(
            bus.q_table
                .get(&("s1".to_string(), "a1".to_string()))
                .copied()
                .unwrap_or(0.0)
                > 0.0
        );
