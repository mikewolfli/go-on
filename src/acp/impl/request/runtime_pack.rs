...
pub(super) async fn handle_governance_remediate(
    ...
    let action_taken = match risk_id.as_str() {
        rid if rid.contains("pua") || rid.contains("PUA") => {
            ...
            // Actually reset the PUA enforcement plan to defaults
...
pub(super) async fn handle_governance_config_save(
    ...
    // Update the governance audit config — convert policy through the
    // HarnessBus interface rather than mutating through Arc.
    if server.harness_bus.is_some() {
        tracing::info!(
    ...
    // NOTE: Changes are applied to in-memory runtime state only.
    // Disk persistence of governance config is a future enhancement.
    // This is acceptable for runtime settings since they are meant to be
    // ephemeral; a future implementation would serialize GovernancePolicy
    // back to config.toml via AppConfig.
    tracing::debug!(
