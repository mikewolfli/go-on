...
impl AcpErrorCode {
    ...
    pub fn from_code(code: i32) -> Option<Self> {
        match code {
            x if x == Self::MethodNotFound as i32 => Some(Self::MethodNotFound),
            x if x == Self::InvalidParams as i32 => Some(Self::InvalidParams),
            x if x == Self::AuthRequired as i32 => Some(Self::AuthRequired),
            x if x == Self::InternalError as i32 => Some(Self::InternalError),
            x if x == Self::ConsultationBlocked as i32 => Some(Self::ConsultationBlocked),
...
/// The ACP/A2A JSON-RPC method table, kept as a module-level constant so
/// tests can assert the sorted invariant against the same list the hot path
/// uses (alphabetical ordering is required for `binary_search`).
