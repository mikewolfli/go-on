    ...
    if let Some(ref harness) = server.governance_deps.harness_bus {
        ...
        spawn_background_task(
            async move {
                ...
                // 10s interval = 2/3 of the 15s heartbeat timeout: a healthy node
                // reports every cycle so its elapsed time stays under the timeout;
                // a node that stops reporting is detected Offline after a few
                // cycles (see check_heartbeats).
                ...
                loop {
                    ...
                    // Run the recovery cycle BEFORE reporting this cycle's
                    // heartbeats: detection must observe the heartbeats reported
                    // by the previous cycle, otherwise a fresh report resets
                    // every node's elapsed time to ~0 and nothing is ever
                    // detected Offline. The heartbeat signal is generated by
                    // this same loop for every registered node (agents were
                    // registered in ServerBuilder::build) — a node whose
                    // heartbeats stop (e.g. a hung worker) is therefore
                    // detected as Offline and a FaultEvent is created.
                    let summary = ft.run_recovery_cycle().await;
                    let mut heartbeat_tasks = Vec::with_capacity(node_names.len());
                    for name in &node_names {
                        let ft = Arc::clone(&ft);
                        let name = name.clone();
                        heartbeat_tasks.push(async move {
                    ...
                    futures_util::future::join_all(heartbeat_tasks).await;
