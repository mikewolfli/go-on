//! GLM (智谱AI) agent implementation
//!
//! This module provides an implementation for the Zhipu AI GLM API.
...
impl GlmAgent {
    ...
    async fn chat_once(
        ...
        let api_key = resolve_secret(&self.api_key_env, "glm.api_key_env")?;
        ...
        if !response.status().is_success() {
            ...
            anyhow::bail!("glm chat request failed with {status}: {body}");
        }
...
impl Agent for GlmAgent {
    async fn chat(
        ...
        Err(last_error.unwrap_or_else(|| anyhow::anyhow!("glm request failed")))
    }
}
