//! StepFun (阶跃星辰) agent implementation
//!
//! This module provides an implementation for the StepFun AI API.
...
impl StepFunAgent {
    ...
    async fn chat_once(
        ...
        let api_key = resolve_secret(&self.api_key_env, "stepfun.api_key_env")?;
        ...
        if !response.status().is_success() {
            ...
            anyhow::bail!("stepfun chat request failed with {status}: {body}");
        }
...
impl Agent for StepFunAgent {
    async fn chat(
        ...
        Err(last_error.unwrap_or_else(|| anyhow::anyhow!("stepfun request failed")))
    }
}
