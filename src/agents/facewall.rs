//! FaceWall (面壁智能) agent implementation
//!
...
impl FaceWallAgent {
    ...
    async fn chat_once(
        ...
        if !response.status().is_success() {
            ...
            anyhow::bail!("facewall chat request failed with {status}: {body}");
        }
...
impl Agent for FaceWallAgent {
    async fn chat(
        ...
        Err(last_error.unwrap_or_else(|| anyhow::anyhow!("facewall request failed")))
    }
}
