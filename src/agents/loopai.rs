//! LoopAI (循环智能) agent implementation
//!
...
impl LoopAiAgent {
    ...
    async fn chat_once(
        ...
        if !response.status().is_success() {
            ...
            anyhow::bail!("loopai chat request failed with {status}: {body}");
        }
...
impl Agent for LoopAiAgent {
    async fn chat(
        ...
        Err(last_error.unwrap_or_else(|| anyhow::anyhow!("loopai request failed")))
    }
}
