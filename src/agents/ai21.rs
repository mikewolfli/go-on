//! AI21 Labs agent implementation
//!
//! This module provides an implementation for the AI21 Labs API.
...
impl Ai21Agent {
    ...
    async fn chat_once(
        ...
        let api_key = resolve_secret(&self.api_key_env, "ai21.api_key_env")?;
        ...
        if !response.status().is_success() {
            ...
            anyhow::bail!("ai21 chat request failed with {status}: {body}");
        }
...
impl Agent for Ai21Agent {
    async fn chat(
        ...
        Err(last_error.unwrap_or_else(|| anyhow::anyhow!("ai21 request failed")))
    }
}
