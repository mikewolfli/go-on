//! DeepQuest (深度求索) agent implementation
//!
//! This module provides an implementation for the DeepQuest AI API.
...
impl DeepQuestAgent {
    ...
    async fn chat_once(
        ...
        let api_key = resolve_secret(&self.api_key_env, "deepquest.api_key_env")?;
        ...
        if !response.status().is_success() {
            ...
            anyhow::bail!("deepquest chat request failed with {status}: {body}");
        }
...
impl Agent for DeepQuestAgent {
    async fn chat(
        ...
        Err(last_error.unwrap_or_else(|| anyhow::anyhow!("deepquest request failed")))
    }
}
