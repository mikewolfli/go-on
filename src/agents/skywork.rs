//! Skywork (昆仑万维) agent implementation
//!
...
impl SkyworkAgent {
    ...
    async fn chat_once(
        ...
        if !response.status().is_success() {
            ...
            anyhow::bail!("skywork chat request failed with {status}: {body}");
        }
...
impl Agent for SkyworkAgent {
    async fn chat(
        ...
        Err(last_error.unwrap_or_else(|| anyhow::anyhow!("skywork request failed")))
    }
}
