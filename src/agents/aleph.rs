//! Aleph Alpha agent implementation
//!
//! This module provides an implementation for the Aleph Alpha API.
...
impl AlephAgent {
    ...
    async fn chat_once(
        ...
        let api_key = resolve_secret(&self.api_key_env, "aleph.api_key_env")?;
        ...
        if !response.status().is_success() {
            ...
            anyhow::bail!("aleph chat request failed with {status}: {body}");
        }
...
impl Agent for AlephAgent {
    async fn chat(
        ...
        Err(last_error.unwrap_or_else(|| anyhow::anyhow!("aleph request failed")))
    }
}
