//! Langboat (澜舟科技) agent implementation
//!
...
impl LangboatAgent {
    ...
    async fn chat_once(
        ...
        if !response.status().is_success() {
            ...
            anyhow::bail!("langboat chat request failed with {status}: {body}");
        }
...
impl Agent for LangboatAgent {
    async fn chat(
        ...
        Err(last_error.unwrap_or_else(|| anyhow::anyhow!("langboat request failed")))
    }
}
