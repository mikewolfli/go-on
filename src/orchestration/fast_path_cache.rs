...
pub struct FastPathCache {
    ...
    /// Cache prerequisites hash → `EnvCacheValue`.
    env_cache: Mutex<HashMap<u64, CacheEntry<EnvCacheValue>>>,
    /// Cache task type → `RouteTemplate`.
...
impl FastPathCache {
    ...
    pub fn set_intent(&self, task_text: &str, value: IntentCacheValue) {
        ...
        let mut cache = self
            ...
            .lock()
            .expect("intent_cache lock poisoned");
        Self::evict_if_needed(&mut cache, self.max_entries);
    ...
    pub fn get_skills(&self, task_text: &str) -> Option<SkillCacheValue> {
        let key = Self::fingerprint(task_text);
        let mut cache = self.skill_cache.lock().expect("skill_cache lock poisoned");
        if let Some(entry) = cache.get_mut(&key) {
            if entry.created_at.elapsed() < self.ttl {
                entry.hit_count += 1;
    ...
    pub fn register_route(&self, template: RouteTemplate) {
        let mut cache = self.route_cache.lock().expect("route_cache lock poisoned");
        cache.insert(template.task_type.clone(), template);
