...
多用户服务器模式（`profile-multi-users-server`）是 go-on 的企业级部署配置，专为具有多个并发用户的生产环境设计。它使用 PostgreSQL 和 pgvector 进行可扩展存储，并提供安全、监控和高可用性的高级功能。
...
- **企业监控**：全面的可观测性堆栈
- **可扩展性**：水平扩展能力
- **完整 Phase 4 架构**：所有 14 条总线和 21 个 F-GAP 模块
...
[protocol]
mode = "mcp_http"  # 多用户访问的 MCP over HTTP
